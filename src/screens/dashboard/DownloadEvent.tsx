...
const DownloadEvent = ({}) => {
  ...
  const fetch = async () => {
    try {
      const response = await axios.get(
        `https://achev.ca/wp-json/custom/v1/events`
      );
  ...
  console.log("objemail==>", obj?.email);
  const sendEmail = () => {
    ...
    homeStack.navigate("RegiteredForEvent", {
      img: getEvent?.image?.url,
  ...
  console.log("getEventID----->>>>>", getEvent?.id);
