...
type Props = NativeStackScreenProps<AuthRoutes, "PasswordResetOTP">;
...
const OTPScreen = ({ route, navigation }: Props) => {
  ...
  const [showErrorNote, setShowErrorNote] = useState<boolean>(false);
  ...
  const handleOtpSubmit = async () => {
    ...
    if (otpValue.length === CELL_COUNT) {
      try {
        ...
        if (response.data === 1) {
          globalSuccessMessageVariable({ message: "Activation successful" });
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.OTPContainer}>
          ...
          <View style={{ paddingHorizontal: 50 }}>
            {showErrorNote ? (
              <HelperText
                styles={{ marginTop: 3 }}
                label={
                  'The 4-digit code you have entered is incorrect. Please enter the code again or resend a new code using the "Resend Code" button.'
                }
              />
            ) : null}
          </View>
          ...
          >
            ...
            <TextLinkButton
              title={"Resend Code"}
              labelStyle={{ color: "#776E64", fontSize: 16, fontWeight: "700" }}
              onPress={Resend}
            />
          </View>
        </View>
