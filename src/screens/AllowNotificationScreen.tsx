...
const AllowNotificationScreen = () => {
  ...
  const handleTogglePush = async (value: boolean) => {
    ...
    if (value) {
      // User selected to allow notifications
      NotifiAllowed(true);
  ...
  useEffect(() => {
    const setNotificationVisited = async () => {
      try {
        await AsyncStorage.setItem("NotifiScreenVisited", "true");
        NotifiScreenVisited(true);
      } catch (error) {
        console.error("Error storing notification screen visit:", error);
      }
    };
