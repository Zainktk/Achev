...
type Props = NativeStackScreenProps<AuthRoutes, "ConfirmPasswordScreen">;
...
const UpdateOtp = ({ route, navigation }: Props) => {
    ...
    if (otpValue.length === CELL_COUNT) {
      try {
        ...
        const response = await axios({
          ...
          data: {
            Email: route.params.email, // Email from route params
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.OTPContainer}>
          <View style={{ paddingHorizontal: 20, gap: 30 }}>
            <ScreenTitle label={"Enter confirmation code"} />
            <View>
              <Text
                style={{
